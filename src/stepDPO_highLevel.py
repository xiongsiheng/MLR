...
# Output dirs
OUT_DIR = f"{args.output_dir}/{model_name.split('/')[-1]}_DPO_highLevel"
...
for name in dataset_names:
    folder_path = f"../data/{name}_stepDPO/train_highLevel.jsonl"
...
# Load adapters and make them trainable
lora_dir = f"{args.output_dir}/{model_name.split('/')[-1]}_highLevel_lora_SFT"
model = PeftModel.from_pretrained(model, lora_dir, is_trainable=True)
model.enable_input_require_grads()  # good practice for LoRA + HF/TRL
...
training_args = DPOConfig(
    ...
    max_length=4224,                     # total (prompt + response)
    beta=0.1,
)
