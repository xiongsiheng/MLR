...
log_file = f"{output_dir}/{model_name.split('/')[-1]}_compressor_SFT_training_log.txt"
save_path = f"{output_dir}/{model_name.split('/')[-1]}_compressor_SFT.safetensors"
...
# ----- Training Setup -----
if f_read_data:
    samples = load_compression_samples(dataset_names)
...
if f_train:  # Use Libraries Supporting Model Parallelism
    # Train for the summarization task.
    ...
    # Prepare the model, optimizer with Accelerate.
    model, optimizer = accelerator.prepare(model, optimizer)
    model.train()

    # Create a dataset for the current stage.
    train_dataset = MLR_Dataset(samples, tokenizer, num_samples=None, min_seq_length=min_seq_length, max_seq_length=max_seq_length, f_print=False)
    ...
    # If needed, prepare the dataloader with Accelerate.
    ...
    for epoch in range(num_epochs):
        ...
        if cnt_batch % 500 == 0 or cnt_batch == len(train_loader):
